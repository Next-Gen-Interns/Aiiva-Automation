...
<>
    ...
    <div
      ...
      <div className="w-[100%] bg-[white] mx-auto px-4 py-3 flex items-center justify-between rounded-2xl shadow-sm">
        ...
        <nav className="w-[33%] lg:items-center lg:justify-center hidden lg:flex gap-2 flex-shrink-0">
          ...
          <div className="lg:w-[33%] flex items-center justify-end gap-1 md:gap-3 lg:gap-4">
            ...
            <button
              ...
              className="relative flex items-center justify-center w-9 h-9 rounded-full border border-black/25 hover:border-black/60"
            >
              🛒
              {cart.length > 0 && (
        ...
        {/* ── Mobile drawer ── */}
        <div
          className={`drawer fixed top-0 left-0 w-[100%] border-b bg-white/95 border-[black]/[0.4] z-[999] flex flex-col overflow-y-auto shadow-[4px_0_40px_rgba(0,0,0,0.6)] lg:hidden ${menuOpen ? "open" : ""}`}
        >
        
          {/* Drawer head */}
          <div className="flex items-center justify-between px-5 py-4 bg-white/95 border-b border-[black]/[0.07]">
            <Link
              href="/"
